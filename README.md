...
- **均匀覆盖整个对话过程**：只上传约 4000 字节的抽样——前 2 条与后 2 条必发、中间等距挑选，**单条请求保持完整不截断**，无论对话多长，成本恒定；
...
每次触发一次辅助调用（输入 ≤4096 字节 ≈ 1300~2000 tokens，输出 ≤64 tokens）。

| 模型（deepseek-v4-flash 官方价，2026-07） | 单次成本 | 50 轮/天月度成本 |
|---|---|---|
| 高峰时段 | ≈ $0.001（约 0.007 元） | ≈ 10 元 |
| 非高峰时段 | ≈ $0.0005（约 0.0035 元） | ≈ 5 元 |

输入预算恒定，成本不随对话长度增长；若会话路由为 deepseek-v4-pro 约为 3 倍。
...
| 常量 | 默认 | 含义 |
|---|---|---|
| `BUDGET_BYTES` | 4096 | 抽样输入预算（UTF-8 字节） |

...
export function apply(ctx) {
	// ── 配置常量（无设置界面，改这里即可）──
	const BUDGET_BYTES = 4096; // 抽样输入预算（UTF-8 字节），与官方 maxInputBytes 一致
	...
	ctx.on("session/event", (session, event) => {
		...
		handleNewMessage(session, event.seq);
	});

	function handleNewMessage(session, seq) {
		...
		runGeneration(session, messages, last.seq)
			...
			.finally(() => {
				...
				if (state.pending) {
					...
					if (l && l.seq !== last.seq) handleNewMessage(session, l.seq);
				}
			});
	}
